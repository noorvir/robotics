...
data = theta_closedform;
save('youbot_joint_data.mat', 'data');


% Inverse kinematics - Iterative method
tic;
use_CF_q4 = true;
theta_jacob = inverse_kine_jacobian(youbot_traj, Youbot, use_CF_q4, 'q4');
cost_jacob = toc;

% Inverse kinematics - Iterative method (Peter Corke's toolbox)
tic;
% theta_ikine = zeros(size(youbot_traj,1), 5);
alpha = 0.05;
tol = 0.01;
theta_ikine = inverse_kine_PCRT(youbot_traj, Youbot, alpha, tol, 'q4');
cost_ikine = toc;
...
view(0, 90);
hold on
Youbot.plot(theta_closedform, 'trail', 'r-','delay', 0.01)
hold off
...
view(0, 90);
hold on
Youbot.plot(theta_jacob, 'trail', 'r-','delay', 0.01)
hold off
...
view(0, 90);
hold on
Youbot.plot(theta_ikine, 'trail', 'r-','delay', 0.01)
hold off
